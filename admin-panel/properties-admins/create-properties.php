...
<?php 
  ...
  if(!isset($_SESSION['username'])) {
    echo "<script>window.location.href='http://localhost/homeland/admin-panel/admins/login-admins.php' </script>";
  }
  ...
  if(isset($_POST['submit'])) {
    ...
    OR empty($_POST['description']) OR empty($_POST['price_sqft'])) {
      ...
      $insert = $conn->prepare("INSERT INTO properties (name, location, price, beds, baths,
      ...
      :year_built, :type, :description, :price_sqft, :admin_name, :image)");
    ...
    foreach ($_FILES['image']['tmp_name'] as $key => $value) {
      ...
      move_uploaded_file($filename_tmp, 'thumbnails/'.$newfilename);
      $finalimg=$newfilename;
        

             
      $insertqry=$conn->prepare("INSERT INTO relatedimages( image, property_id) VALUES ('$finalimg','$id')");
      $insertqry->execute();

      echo "<script>window.location.href='http://localhost/homeland/admin-panel/properties-admins/show-properties.php' </script>";
